...
@Component({
  selector: 'app-womens-weight-heaping-second-decimal-memberwise',
  templateUrl: './womens-weight-heaping-second-decimal-memberwise.component.html',
  styleUrls: ['./womens-weight-heaping-second-decimal-memberwise.component.scss']
})
export class WomensWeightHeapingSecondDecimalMemberwiseComponent {
  ...
  ngOnInit() { 
    ...
    this.cols = [                
      ...
      { field: 'percentWithSecondDecimal_0_or_5', header: 'Percent With Second Decimal 0 to 5' },
      { field: 'numberOfWomen', header: 'Number Of Women' },
  ...
  async getdata(){
     this.datalist = await lastValueFrom(this.householdService.getAllWomensWeightHeapingSecondDecimalMemberwise(this.stateId,this.teamNo));
...
exportPdf() {
    import('jspdf').then((jsPDF) => {
        import('jspdf-autotable').then((x) => {
            ...
            doc.save('womens-weight-heaping-second-decimal-memberwise.pdf');
        });
    });
}
...
exportExcel() {
    import('xlsx').then((xlsx) => {
        ...
        this.saveAsExcelFile(excelBuffer, 'womens-weight-heaping-second-decimal-memberwise-');
    });
}
