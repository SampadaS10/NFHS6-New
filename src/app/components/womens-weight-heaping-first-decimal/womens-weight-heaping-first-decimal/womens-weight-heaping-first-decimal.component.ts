...
@Component({
  selector: 'app-womens-weight-heaping-first-decimal',
  templateUrl: './womens-weight-heaping-first-decimal.component.html',
  styleUrls: ['./womens-weight-heaping-first-decimal.component.scss']
})
export class WomensWeightHeapingFirstDecimalComponent{
  ...
  async getdata(){
     this.datalist = await lastValueFrom(this.householdService.getAllWomensWeightHeapingFirstDecimal());
...
exportPdf() {
    import('jspdf').then((jsPDF) => {
        import('jspdf-autotable').then((x) => {
            ...
            doc.save('womens-weight-heaping-first-decimal.pdf');
        });
    });
}
...
exportExcel() {
    import('xlsx').then((xlsx) => {
        ...
        this.saveAsExcelFile(excelBuffer, 'womens-weight-heaping-first-decimal-');
    });
}
