...
@Component({
  selector: 'app-mens-height-heaping-first-decimal-memberwise',
  templateUrl: './mens-height-heaping-first-decimal-memberwise.component.html',
  styleUrls: ['./mens-height-heaping-first-decimal-memberwise.component.scss']
})
export class MensHeightHeapingFirstDecimalMemberwiseComponent 
{
  ...
  ngOnInit() {  
    ...
    this.cols = [                
      ...
      { field: 'numberOfMen', header: 'Number Of Men 15-54 with Valid Height Data' },
  ...
  async getdata(){
     this.datalist = await lastValueFrom(this.householdService.getAllMensHeightHeapingFirstDecimalMemberwise(this.stateId,this.teamNo));
...
exportPdf() {
    import('jspdf').then((jsPDF) => {
        import('jspdf-autotable').then((x) => {
            ...
            doc.save('mens-height-heaping-first-decimal-memberwise.pdf');
        });
    });
}
...
exportExcel() {
    import('xlsx').then((xlsx) => {
        ...
        this.saveAsExcelFile(excelBuffer, 'mens-height-heaping-first-decimal-memberwise_');
    });
}
