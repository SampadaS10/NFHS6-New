...
@Component({
  selector: 'app-age-at-death-heaping-teamwise',
  templateUrl: './age-at-death-heaping-teamwise.component.html',
  styleUrls: ['./age-at-death-heaping-teamwise.component.scss']
})
export class AgeAtDeathHeapingTeamwiseComponent implements OnInit {
...
async getdata(){
  this.datalist = await lastValueFrom(this.householdService.getAllAgeAtDeathHeapingTeamwise(this.stateId));
...
exportPdf() {
  import('jspdf').then((jsPDF) => {
      import('jspdf-autotable').then((x) => {
          ...
          doc.save('Age-At-Death-Heaping-Teamwise.pdf');
      });
  });
}
...
exportExcel() {
  import('xlsx').then((xlsx) => {
      ...
      this.saveAsExcelFile(excelBuffer, 'Age-At-Death-Heaping-Teamwise-');
  });
}
