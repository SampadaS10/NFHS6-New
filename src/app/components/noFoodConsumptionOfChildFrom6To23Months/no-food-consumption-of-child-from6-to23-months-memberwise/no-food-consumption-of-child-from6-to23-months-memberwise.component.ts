...
@Component({
  selector: 'app-no-food-consumption-of-child-from6-to23-months-memberwise',
  templateUrl: './no-food-consumption-of-child-from6-to23-months-memberwise.component.html',
  styleUrls: ['./no-food-consumption-of-child-from6-to23-months-memberwise.component.scss']
})
export class NoFoodConsumptionOfChildFrom6To23MonthsMemberwiseComponent implements OnInit{
  constructor(
...
exportColumns :any[]=[];
  ...
  ngOnInit() {
   ...
   this.cols = [                
    ...
    this.householdService.getAllNoFoodConsumptionOfChildFrom6To23MonthsMemberwise(this.stateId,this.teamNo).subscribe(
      (datalist) => {
...
exportPdf() {
  import('jspdf').then((jsPDF) => {
      import('jspdf-autotable').then((x) => {
          ...
          doc.save('No-Food-Consumption-Of-Child-From-6To23-Months-memberwise.pdf');
      });
  });
}
...
exportExcel() {
  import('xlsx').then((xlsx) => {
      ...
      this.saveAsExcelFile(excelBuffer, 'No-Food-Consumption-Of-Child-From-6To23-Months-memberwise_');
  });
}
